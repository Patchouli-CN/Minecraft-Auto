...
_DEFAULT_PATTEN_TEMP: list[EventRegexRules] = [
    {
        "name": "游戏崩溃",
        "rules": [
            {"regex": "\\[\\d{2}:\\d{2}:\\d{2}\\] \\[.*FATAL\\]:.*", "groups": []},
            {
        ...
        "rules": [
            {
                ...
                "groups": ["land_name", "player", "in_value", "now_value"],
            }
        ],
    },
    {
        ...
        "rules": [
            {
                ...
                "groups": ["land_name", "player", "out_value", "now_value"],
            }
        ],
    },
    {
        ...
        "rules": [
            {
                ...
                "groups": ["server_name", "channel", "tag", "player", "content"],
            }
        ],
    },
    {
    ...
    },
    {
        "name": "加入",
        "rules": [
            {"regex": "(?P<player>\\w+) joined the game", "groups": ["player"]},
            {"regex": "(?P<player>\\w+) 加入了游戏", "groups": ["player"]},
        ],
    },
    {
        "name": "退出",
        "rules": [
            {"regex": "(?P<player>\\w+) left the game", "groups": ["player"]},
            {"regex": "(?P<player>\\w+) 退出了游戏", "groups": ["player"]},
        ],
    },
    {"name": "踢出", "rules": [{"regex": "您已被踢出", "groups": []}]},
    {
        "name": "断开",
        "rules": [
            {
    ...
    {"name": "挂机", "rules": [{"regex": "你暂时离开了", "groups": []}]},
    {"name": "挂机恢复", "rules": [{"regex": "你回来了", "groups": []}]},
]
    ...
    try:
        if not PATTERN_FILE.exists():
            PATTERN_FILE.write_text(
                json.dumps(_DEFAULT_PATTEN_TEMP, ensure_ascii=False, indent=2),
                encoding="utf-8",
            )
        ...
        CONFIG_FILE.write_text(
            json.dumps(_DEFAULT_CONF, ensure_ascii=False, indent=2), encoding="utf-8"
        )
