...
class Registry:
    ...
    def _init_builtins(self):
        ...
        builtins = [
            ...
            (
                bytes,
                lambda b: {"__bytes__": b.hex()},
                lambda d: bytes.fromhex(d["__bytes__"]),
            ),
            (set, list, set),
            (frozenset, list, frozenset),
            (tuple, lambda t: {"__tuple__": list(t)}, lambda d: tuple(d["__tuple__"])),
        ]
...
class Serializer(_BaseHandler):
    ...
    def serialize(
        self, obj: Any, target_type: Any = None, seen: Optional[set[int]] = None
    ) -> Any:
        """序列化对象"""
        if obj is None:
            return None
    ...
    def _serialize_container(
        self, obj: Any, actual_type: Any, seen: Optional[set[int]]
    ) -> Any:
        """序列化容器对象"""
...
class Deserializer(_BaseHandler):
    ...
    def _deserialize_marked(self, marker: tuple[str, Any], main_type: Any) -> Any:
        """处理带标记的数据"""
        mark, value = marker
